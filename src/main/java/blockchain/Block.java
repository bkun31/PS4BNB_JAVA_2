...
public class Block {
	...
	public String hashBlock(int difficulte) {
		...
		hashBlock = HashUtil.applySha256(hashPrecedent + merkel + dateToString(date) + Nonce + numBlock + stringListeTx() + user.getNom()); // Concatenation des info

		while (!hashBlock.substring(0, difficulte).equals(dif)) { // Verifie que les 4 premier caracter du hash soit "0"
																	// x difficulte
			Nonce++;
	...
	public String merkelHash(String tabTx[], int nbTx) {
		...
		if (nbTx % 2 != 0) {
			tabHash[nbTxHash] = HashUtil.applySha256(tab[i] + tab[i]); // Si nbTx impaire concatene et hash 2 fois la
