...
public class BlockChain {
	...
	public void minage(User user) {
		 ...
		 * source du montant de la tx + les frais �ventuelle
		 */
		tx = queue.poll();
		...
		if (utxoInputs != null) {
			 ...
			 * récompense si phase déflation (plus bas)
			 */
			tabTx[0] = market;
			tabTx[1] = fees;
	 ...
	 * @return User[] tableau de User de la transaction en paramètre
	 */
	...
	/**
	 * @param String transaction
	 * @return Long montant de la transaction en paramètre
	 */
	...
	private ArrayList<TxInputs> searchUtxo(User user, long montant) {
		...
		ArrayList<TxOutputs> sup = new ArrayList<TxOutputs>();
		int i = 0;
		long solde = 0;
		...
		for (i = 0; i < sup.size(); i++) {
			globalUtxoList.remove(sup.get(i));
		}
		return liste;
	}
	...
	public void setDifficulte(int difficulte) {
		if (difficulte > 32 || difficulte < 0) {
			System.out.println("Difficult� invalide");
		} else {
